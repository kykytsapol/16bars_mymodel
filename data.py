...
class PolyphemusDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pitch_tensor = torch.tensor(constants.lst, device=pitches.device)

        # Create a mask indicating where elements in onehot_p match elements in reshaped_pitches
        mask = pitches.reshape(-1, 1) == pitch_tensor

        # Convert the mask to binary values (0 or 1)
        onehot_p = mask.to(torch.float)
